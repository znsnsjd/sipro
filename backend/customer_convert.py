...
async def deal_scheme_kind(org: str, deal: dict) -> str:
    """Jenis skema yang DIPILIH SALES saat reservasi (`deal.scheme_id` → `payment_schemes.kind`)."""
    if not deal.get("scheme_id"):
        return None
    sch = await db.payment_schemes.find_one({"id": deal["scheme_id"], "org_id": org},
                                            {"_id": 0, "kind": 1, "type": 1})
    kind = (sch or {}).get("kind") or (sch or {}).get("type")
    return kind if kind in ce.SCHEMES else None


async def suggested_scheme(org: str, deal: dict) -> str:
    """Skema kontrak = jenis skema pembayaran yang dipilih sales saat reservasi.

    Jalur 1: dulu fungsi ini MENEBAK (ada pengajuan KPR → kpr; AR > 2 baris → cash bertahap)
    — dan AR-nya sendiri berasal dari skema bawaan finance, sehingga tebakan itu melahirkan
    varian SPR yang salah. Tebakan hanya dipakai bila deal lama tidak menyimpan `scheme_id`.
    """
    kind = await deal_scheme_kind(org, deal)
    if kind:
        return kind
...
async def preview(org: str, deal_id: str) -> dict:
    ...
    return {
        ...
        # Jalur 1: skema yang dipilih sales saat reservasi MENGUNCI jenis kontrak & varian SPR.
        "deal_scheme": await deal_scheme_kind(org, deal),
        "deal_scheme_name": (await db.payment_schemes.find_one(
            {"id": deal.get("scheme_id")}, {"_id": 0, "name": 1}) or {}).get("name")
...
async def _repoint_deal_scheme(org: str, deal: dict, kind: str, actor: str) -> dict:
    """Deal berskema BAWAAN dikonversi ke jenis lain → deal memakai skema jenis itu, AR disusun ulang."""
    import payment_scheme_engine as psx
    import finance_engine as fin
    hasil = await psx.resolve_for_contract(org, {"scheme": kind, "project_id": deal.get("project_id")})
    sch = hasil.get("scheme") or await ce.ensure_payment_scheme(org, kind)
    ts = now_iso()
    await db.deals.update_one({"id": deal["id"]}, {"$set": {
        "scheme_id": sch["id"], "scheme_explicit": True, "updated_at": ts},
        "$push": {"scheme_history": {"from": deal.get("scheme_id"), "to": sch["id"], "at": ts,
                                     "by": actor, "reason": "konversi pembeli: jenis kontrak "
                                     f"{ref.label_of('payment_scheme', kind)}"}}})
    deal = await db.deals.find_one({"id": deal["id"]}, {"_id": 0})
    if await db.ar_invoices.find_one({"org_id": org, "deal_id": deal["id"]}, {"_id": 0, "id": 1}):
        await fin.create_ar_for_deal(deal, scheme_id=sch["id"], org_id=org, replace=True, actor=actor)
    return deal
...
async def convert(org: str, deal_id: str, actor: str, payload: dict = None) -> dict:
    ...
    # Jenis skema kontrak WAJIB sama dengan skema yang DIPILIH sales saat reservasi — AR dan
    # SPR dibangun dari skema itu; kontrak berjenis lain akan menerbitkan dokumen yang salah.
    # Skema yang hanya BAWAAN (sales tidak memilih) boleh diganti di sini: deal diarahkan ke
    # skema jenis kontrak dan jadwal tagihan disusun ulang, supaya AR ↔ kontrak ↔ SPR satu.
    deal_kind = await deal_scheme_kind(org, deal)
    if deal_kind and scheme != deal_kind:
        if deal.get("scheme_explicit"):
            raise ValueError(
                f"Skema pembayaran deal ini adalah {ref.label_of('payment_scheme', deal_kind)} "
                f"(dipilih saat reservasi), bukan {ref.label_of('payment_scheme', scheme)}. "
                "Ubah skema pembayaran deal lebih dahulu bila memang berubah.")
        deal = await _repoint_deal_scheme(org, deal, scheme, actor)
    ts = now_iso()
    ...
    import finance_engine as fin
    try:
        await fin.create_ar_for_deal(deal, scheme_id=deal.get("scheme_id"), org_id=org, actor=actor)
