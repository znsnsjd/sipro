...
async def reserve(org: str, *, lead: dict, unit: dict, actor: str, booking_fee: int = 0,
    ...
    ts = now_iso()
    deal_id = new_id()
    hold_days = int(await cfg.get("reservation.hold_days", org_id=org) or BOOKING_HOLD_DAYS)
    reserved_until = due_in(days=hold_days)
    ...
    # Fase 69B: booking fee = komponen pembayaran terpisah → tagihan INV-BF lahir bersamaan.
    import booking_fee as bf
    inv = await bf.create_invoice(org, deal, actor)
    if inv:
        deal.update({"booking_fee_status": "unverified", "booking_fee_invoice_id": inv["id"]})
